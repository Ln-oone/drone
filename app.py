...
def find_optimal_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius=5, side="right"):
    ...
    for attempt in range(1, 12):
        ...
        for t in t_vals:
            if start[1] < min_lat and end[1] > max_lat:
                ...
                lat = start[1] + (target - start[1]) * t
            if dy != 0:
                t_orig = (lat - start[1]) / dy
                t_orig = max(0, min(1, t_orig))
            else:
                t_orig = t
            orig_x = start[0] + dx * t_orig
            waypoints.append([orig_x + perp_x * offset_deg_lng, lat])

        candidate = [start] + waypoints + [end]
        path_safe = True
        for i in range(len(candidate) - 1):
            if not is_path_segment_clear(candidate[i], candidate[i + 1], blocking_obs, flight_altitude, safety_radius):
                path_safe = False
                break
    ...
    for t in [0.1, 0.25, 0.4, 0.6, 0.75, 0.9]:
        ...
        orig_x = start[0] + dx * t_orig
        if side == "right":
            lng = max_lng + offset_deg_lng * (0.8 + 0.2 * math.sin(math.pi * t))
        else:
            lng = min_lng - offset_deg_lng * (0.8 + 0.2 * math.sin(math.pi * t))
        fallback.append([lng, lat])
    return [start] + fallback + [end]

def find_left_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius=5):
    return find_optimal_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius, "left")

def find_right_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius=5):
    return find_optimal_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius, "right")

def find_best_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius=5):
    if is_path_segment_clear(start, end, obstacles_gcj, flight_altitude, safety_radius):
        return [start, end]
    left = find_left_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius)
    right = find_right_avoidance_path(start, end, obstacles_gcj, flight_altitude, safety_radius)
    left_len = sum(distance(left[i], left[i + 1]) for i in range(len(left) - 1)) * 111000
    right_len = sum(distance(right[i], right[i + 1]) for i in range(len(right) - 1)) * 111000
...
def create_planning_map(center_gcj, points_gcj, obstacles_gcj, flight_history=None, planned_path=None,
                        straight_blocked=True, flight_altitude=50, drone_pos=None, direction="最佳航线", safety_radius=5):
    ...
    if planned_path and len(planned_path) > 1:
        path_locs = [[p[1], p[0]] for p in planned_path]
        line_color = "purple" if "向左" in direction else "orange" if "向右" in direction else "green"
        folium.PolyLine(path_locs, color=line_color, weight=5, opacity=0.9, popup=f"✈️ {direction}").add_to(m)
...
# ==================== 障碍物管理页面 ====================
def render_obstacle_management_page(flight_alt, *args):
    st.header("🚧 障碍物管理")
...
def render_obstacle_map_view(flight_alt):
    ...
    draw = plugins.Draw(export=True, position='topleft', draw_options={'polygon': {'allowIntersection': False, 'showArea': True, 'color': '#ff0000', 'fillColor': '#ff0000', 'fillOpacity': 0.4},
    ...
    for obs in st.session_state.obstacles_gcj:
        ...
        if coords and len(coords) >= 3:
            color = "red" if height > flight_alt else "orange"
            popup_text = f"<div><b>🏢 {obs.get('name')}</b><br>高度: {height} 米<br>ID: {obs.get('id', 'N/A')}</div>"
...
def main():
    ...
    if page == "🗺️ 航线规划":
        ...
        render_obstacle_management_page(flight_alt, drone_speed, auto_save)
