...
def get_all_obstacle_extent(obstacles: List[Dict]) -> Tuple[float, float, float, float]:
    """获取所有障碍物的边界范围"""
...
def is_point_safe_all(point: List[float], obstacles: List[Dict], flight_altitude: float, safety_radius: float) -> bool:
    """检查点是否安全（不在任何障碍物内，且距离足够）"""
    for obs in obstacles:
        if obs.get('height', 30) <= flight_altitude:
            continue
...
def is_segment_safe_all(p1: List[float], p2: List[float], obstacles: List[Dict], flight_altitude: float, safety_radius: float) -> bool:
    """检查线段是否安全（不与任何障碍物相交，且保持安全距离）"""
    # 先检查端点
    if not is_point_safe_all(p1, obstacles, flight_altitude, safety_radius):
        return False
    if not is_point_safe_all(p2, obstacles, flight_altitude, safety_radius):
        return False
...
def find_safe_path_dynamic(start: List[float], end: List[float],
    ...
    # 获取所有阻挡障碍物的边界
    min_lng, max_lng, min_lat, max_lat = get_all_obstacle_extent(blocking_obs)
    ...
    if start[1] < min_lat and end[1] > max_lat:
        ...
        waypoint_lats = [
            start[1],
            start[1] + (min_lat - start[1]) * 0.25,
            start[1] + (min_lat - start[1]) * 0.5,
            min_lat - 0.00002,
            (min_lat + max_lat) / 2,
            max_lat + 0.00002,
            end[1] - (end[1] - max_lat) * 0.5,
            end[1] - (end[1] - max_lat) * 0.25,
            end[1]
        ]
        ...
        waypoint_lats = [
            start[1],
            start[1] + (target_lat - start[1]) * 0.25,
            start[1] + (target_lat - start[1]) * 0.5,
            start[1] + (target_lat - start[1]) * 0.75,
            target_lat,
            end[1]
        ]
    elif end[1] > max_lat:
        ...
        waypoint_lats = [
            start[1],
            target_lat,
            end[1] - (end[1] - target_lat) * 0.75,
            end[1] - (end[1] - target_lat) * 0.5,
            end[1] - (end[1] - target_lat) * 0.25,
            end[1]
        ]
    else:
        # 同侧绕过 - 绕行到障碍物上方或下方
        if side == "right":
            target_lat = max_lat + 0.00015
        else:
            target_lat = min_lat - 0.00015
        waypoint_lats = [
            start[1],
    ...
    # 限制绕行点数量
    if len(waypoint_lats) > num_mid_waypoints + 2:
        step = len(waypoint_lats) / num_mid_waypoints
        waypoint_lats = [waypoint_lats[int(i * step)] for i in range(num_mid_waypoints)]
    ...
    for attempt in range(1, 25):
        offset_meters = base_offset_meters + safety_radius * attempt * 0.3
        ...
        for i in range(len(candidate_path) - 1):
            if not is_segment_safe_all(candidate_path[i], candidate_path[i + 1],
                                      obstacles_gcj, flight_altitude, safety_radius):
                path_safe = False
                break
        
        if path_safe:
            if offset_meters < min_offset:
    ...
    # 创建更多绕行点确保安全
    num_fallback = max(6, min(10, len(waypoint_lats)))
...
def find_left_avoidance_path_dynamic(start: List[float], end: List[float], obstacles_gcj: List[Dict],
                                      flight_altitude: float, safety_radius: float = 5) -> List[List[float]]:
    return find_safe_path_dynamic(start, end, obstacles_gcj, flight_altitude, safety_radius, "left")


def find_right_avoidance_path_dynamic(start: List[float], end: List[float], obstacles_gcj: List[Dict],
                                       flight_altitude: float, safety_radius: float = 5) -> List[List[float]]:
    return find_safe_path_dynamic(start, end, obstacles_gcj, flight_altitude, safety_radius, "right")


def find_best_avoidance_path_dynamic(start: List[float], end: List[float], obstacles_gcj: List[Dict],
                                      flight_altitude: float, safety_radius: float = 5) -> List[List[float]]:
    # 检查直线是否安全
    if is_segment_safe_all(start, end, obstacles_gcj, flight_altitude, safety_radius):
        return [start, end]
    
    # 计算左右绕行路径
    left_path = find_left_avoidance_path_dynamic(start, end, obstacles_gcj, flight_altitude, safety_radius)
    right_path = find_right_avoidance_path_dynamic(start, end, obstacles_gcj, flight_altitude, safety_radius)
...
def create_avoidance_path(start: List[float], end: List[float], obstacles_gcj: List[Dict],
    ...
    if direction == "向左绕行":
        return find_left_avoidance_path_dynamic(start, end, obstacles_gcj, flight_altitude, safety_radius)
    elif direction == "向右绕行":
        return find_right_avoidance_path_dynamic(start, end, obstacles_gcj, flight_altitude, safety_radius)
    else:
        return find_best_avoidance_path_dynamic(start, end, obstacles_gcj, flight_altitude, safety_radius)
...
def render_path_strategy(flight_alt: float):
    ...
    if st.button("🔄 重新规划路径", use_container_width=True):
        ...
        if st.session_state.planned_path:
            st.success(f"已按照「{st.session_state.current_direction}」规划路径，{len(st.session_state.planned_path)-2}个绕行点")
        st.rerun()
...
def render_planning_map_view(flight_alt: float, straight_blocked: bool):
    ...
    m = create_planning_map(
        ...
        flight_alt, drone_pos, st.session_state.current_direction, st.session_state.safety_radius)
    ...
    if st.session_state.heartbeat_sim.history:
        ...
        with c1:
            ...
            if total_waypoints > 0:
                ...
                st.metric(label="🎯 当前航点", value=waypoint_display,
        ...
        with c2:
            st.metric(label="💨 飞行速度", value=f"{latest.speed:.1f} m/s", delta=f"{drone_speed}% 系数" if not latest.arrived else "已到达")
        with c3:
            time_display = f"{int(latest.flight_time//60):02d}:{int(latest.flight_time%60):02d}"
            st.metric(label="⏰ 已用时间", value=time_display)

        c4, c5, c6 = st.columns(3)
        with c4:
            distance_text = f"{remaining_distance/1000:.2f} km" if remaining_distance >= 1000 else f"{remaining_distance:.0f} m"
            st.metric(label="📏 剩余距离", value=distance_text if not latest.arrived else "0 m", delta="已到达!" if latest.arrived else None)
        with c5:
            st.metric(label="🕐 预计到达", value=estimated_arrival)
        ...
        with c6:
            ...
            st.metric(label="🔋 电量模拟", value=f"{battery_color} {battery_percentage:.0f}%", delta=f"{latest.voltage:.1f}V")
            ...
            with col_exp1:
                if st.button("📊 导出飞行数据", use_container_width=True):
                    ...
                    if not df.empty:
                        ...
                        st.download_button(label="📥 下载CSV", data=csv, file_name=f"flight_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mime="text/csv")
            with col_exp2:
                if st.button("📊 导出航点数据", use_container_width=True) and st.session_state.planned_path:
                    ...
                    st.download_button(label="📥 下载航点CSV", data=csv, file_name=f"waypoints_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mime="text/csv")
        ...
        with c_ch1:
            ...
            if len(st.session_state.heartbeat_sim.history) > 1:
                speed_data = [{"时间(s)": i * config.HEARTBEAT_INTERVAL, "速度(m/s)": h.speed} for i, h in enumerate(st.session_state.heartbeat_sim.history[:30])]
                st.line_chart(pd.DataFrame(speed_data), x="时间(s)", y="速度(m/s)")
        with c_ch2:
            st.subheader("📏 剩余距离 vs 时间")
            if len(st.session_state.heartbeat_sim.history) > 1:
                dist_data = [{"时间(s)": i * config.HEARTBEAT_INTERVAL, "剩余距离(m)": max(0, h.remaining_distance)} for i, h in enumerate(st.session_state.heartbeat_sim.history[:30])]
...
def display_monitor_map(flight_alt: float, latest):
    ...
    if st.session_state.planned_path and len(st.session_state.planned_path) > 1:
        ...
        folium.PolyLine([[p[1], p[0]] for p in st.session_state.planned_path], color=line_color, weight=3, opacity=0.7,
                       popup=f"规划航线 - {st.session_state.current_direction}").add_to(m)
    folium.Circle(radius=st.session_state.safety_radius, location=[latest.lat, latest.lng], color="blue", weight=2,
                 fill=True, fill_color="blue", fill_opacity=0.2, popup=f"🛡️ 安全半径: {st.session_state.safety_radius}米").add_to(m)
    ...
    if st.session_state.planned_path and len(st.session_state.planned_path) > 2:
        for i, point in enumerate(st.session_state.planned_path[1:-1]):
            folium.CircleMarker([point[1], point[0]], radius=4, color="yellow", fill=True, fill_color="yellow", fill_opacity=0.8,
    ...
    with cols[2]:
        if st.session_state.obstacles_gcj:
            ...
            st.download_button(label="📥 导出配置", data=json.dumps(config_data, ensure_ascii=False, indent=2),
                              file_name=f"obstacles_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", mime="application/json", use_container_width=True)
        else:
