...
def find_left_avoidance_path(
    ...
    for attempt in range(1, 51):
        ...
        waypoint = [
            mid_x + left_x * offset_deg_lng,
            mid_y + left_y * offset_deg_lat
        ]
        
        # 检查两段路径是否都安全
        if is_path_safe(start, waypoint, blocking_obs, flight_altitude) and \
           is_path_safe(waypoint, end, blocking_obs, flight_altitude):
...
def find_right_avoidance_path(
    ...
    for attempt in range(1, 51):
        ...
        mid_y = (start[1] + end[1]) / 2
        waypoint = [
            mid_x + right_x * offset_deg_lng,
            mid_y + right_y * offset_deg_lat
        ]
...
def create_avoidance_path(
    ...
    """
    # 首先检查直线是否安全（所有障碍物高度低于飞行高度或不与航线相交）
    straight_safe = True
    for obs in obstacles_gcj:
        if obs.get('height', 30) > flight_altitude:
            coords = obs.get('polygon', [])
...
def render_flight_params(flight_alt: float):
    """渲染飞行参数（仅显示参数，不包含控制按钮）"""
    col_met1, col_met2, col_met3 = st.columns(3)
    with col_met1:
        st.metric("当前飞行高度", f"{flight_alt} m")
    with col_met2:
        st.metric("速度系数", f"{st.session_state.get('current_drone_speed', 50)}%")
    with col_met3:
    ...
    with col_btn1:
        if st.button("▶️ 开始飞行", use_container_width=True, type="primary"):
            if st.session_state.points_gcj['A'] and st.session_state.points_gcj['B']:
                ...
                if waypoint_count > 0:
                    st.success(f"🚁 飞行已开始！路径中有 {waypoint_count} 个绕行点")
                else:
                    st.success("🚁 飞行已开始！直线飞行")
                st.rerun()
            else:
...
def main():
    ...
    st.session_state.current_drone_speed = drone_speed  # 保存当前速度供其他页面使用
