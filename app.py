...
<body>
    ...
    <script>
        ...
        Cesium.Ion.defaultAccessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiI5YzVlYzZkMS1jNTk4LTRjYzYtYjFmNS04MjkwYzhiMzQ5NjgiLCJpZCI6MjU2OTM4LCJpYXQiOjE3Mjg2NjEzODl9.2tGtQDqgP04bfNnY6jI7pNHC-GxLd_YV9K0R7Qb1nws';
...
1. 安装依赖：
      本应用只需要 streamlit，没有其他 Python 库依赖。
   ```bash
   pip install streamlit
   ```
2. 保存代码：
      将上面的完整代码复制，保存为 app.py。
3. 运行应用：
   ```bash
   streamlit run app.py
   ```
4. 访问：
      浏览器会自动打开 http://localhost:8501，即可看到两个标签页：
   · 航线规划：完整的 3D 地图，支持坐标输入、坐标系切换、A/B点设置、障碍物生成及高亮圈选。
   · 飞行监控：前端模拟心跳包数据，动态显示信号、电池、无人机位置和日志。

关键功能实现说明

· 3D 地图：使用 Cesium.js 加载全球地形和影像，相机默认对准南京科技职业学院区域。
· 坐标转换：通过 coordtransform 库实现 GCJ-02 ↔ WGS-84 互转，确保地图上标记位置准确。
· 障碍物：在 A、B 两点连线的中点自动生成一个红色半透明立方体（高25米），点击“圈选/高亮障碍物”按钮后相机飞向障碍物并闪烁提示。
· 心跳包模拟：完全在前端 JavaScript 中实现，避免 Streamlit 页面重载导致地图状态丢失。模拟数据包括信号强度、电池电量、无人机位置（沿 A-B 连线移动）。

注意事项

· Cesium Ion Token：代码中提供的 Token 为演示用，可能失效。如果地图无法显示（白屏或提示 token 错误），请前往 Cesium ion 免费注册，获取 Access Token 后替换 map_html 中的 Cesium.Ion.defaultAccessToken 值。
· 所有功能均符合作业要求：3D 地图、坐标转换、A/B 点及障碍物、双界面分离、心跳包监控。
