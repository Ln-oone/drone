...
def get_polygon_bounds(polygon: List[List[float]]) -> Optional[Dict]:
    ...
    return {'min_lng': min_lng, 'max_lng': max_lng, 'min_lat': min_lat, 'max_lat': max_lat,
            'center_lng': (min_lng + max_lng) / 2, 'center_lat': (min_lat + max_lat) / 2}
...
def get_polygon_center(polygon: List[List[float]]) -> List[float]:
    if not polygon:
        return [0, 0]
...
def check_safety_radius(drone_pos: List[float], obstacles_gcj: List[Dict], flight_altitude: float, safety_radius: float) -> Tuple[bool, Optional[float], Optional[str]]:
    ...
    for obs in obstacles_gcj:
        ...
        if coords and len(coords) >= 3:
            if point_in_polygon(drone_pos, coords):
                return False, 0.0, obs.get('name', '障碍物')
...
def save_obstacles(obstacles: List[Dict]) -> bool:
    try:
        backup_config()
        data = {'obstacles': obstacles, 'count': len(obstacles), 'save_time': datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 'version': 'v13.2'}
...
def get_obstacle_boundary_points(obstacle: Dict, side: str, flight_altitude: float, safety_radius: float) -> List[List[float]]:
    ...
    max_lng = max(p[0] for p in coords)
    min_lat = min(p[1] for p in coords)
    max_lat = max(p[1] for p in coords)
    center_lat = (min_lat + max_lat) / 2
    safe_lng, safe_lat = meters_to_deg(safety_radius * 2, center_lat)
    if side == "left":
        left_boundary = min_lng
        return [[left_boundary - safe_lng, max_lat + safe_lat], [left_boundary - safe_lng, center_lat], [left_boundary - safe_lng, min_lat - safe_lat]]
    else:
        right_boundary = max_lng
        return [[right_boundary + safe_lng, max_lat + safe_lat], [right_boundary + safe_lng, center_lat], [right_boundary + safe_lng, min_lat - safe_lat]]


def find_safe_waypoints_for_obstacle(obstacle: Dict, start: List[float], end: List[float], side: str, safety_radius: float, flight_altitude: float) -> List[List[float]]:
    boundary_points = get_obstacle_boundary_points(obstacle, side, flight_altitude, safety_radius)
    if not boundary_points:
        return []
    start_distances = [distance(start, point) for point in boundary_points]
    closest_to_start_idx = start_distances.index(min(start_distances))
    end_distances = [distance(end, point) for point in boundary_points]
    closest_to_end_idx = end_distances.index(min(end_distances))
    if closest_to_start_idx <= closest_to_end_idx:
        return boundary_points[closest_to_start_idx:closest_to_end_idx + 1]
    else:
        return boundary_points[closest_to_end_idx:closest_to_start_idx + 1][::-1]


def get_blocking_obstacles_ordered(start: List[float], end: List[float], obstacles_gcj: List[Dict], flight_altitude: float) -> List[Dict]:
    blocking = []
    for obs in obstacles_gcj:
        if obs.get('height', 30) > flight_altitude:
            coords = obs.get('polygon', [])
            if coords and line_intersects_polygon(start, end, coords):
                ...
                blocking.append({'obstacle': obs, 'distance': distance(start, center)})
    blocking.sort(key=lambda x: x['distance'])
    return [item['obstacle'] for item in blocking]


def merge_adjacent_waypoints(waypoints: List[List[float]], min_distance_meters: float = 5.0) -> List[List[float]]:
    if len(waypoints) <= 2:
        return waypoints
    merged = [waypoints[0]]
    for i in range(1, len(waypoints)):
        if distance(merged[-1], waypoints[i]) * 111000 > min_distance_meters:
            merged.append(waypoints[i])
    if merged[-1] != waypoints[-1]:
        merged.append(waypoints[-1])
    return merged


def create_multi_waypoint_avoidance_path(start: List[float], end: List[float], obstacles_gcj: List[Dict], flight_altitude: float, direction: str, safety_radius: float = 5) -> List[List[float]]:
    blocking_obstacles = get_blocking_obstacles_ordered(start, end, obstacles_gcj, flight_altitude)
    if not blocking_obstacles:
        return [start, end]
    
    if direction == "智能绕行":
        left_path = create_multi_waypoint_avoidance_path(start, end, obstacles_gcj, flight_altitude, "向左绕行", safety_radius)
        right_path = create_multi_waypoint_avoidance_path(start, end, obstacles_gcj, flight_altitude, "向右绕行", safety_radius)
        return left_path if calculate_path_length(left_path) <= calculate_path_length(right_path) else right_path
    
    all_waypoints = [start]
    current_start = start
    
    for obstacle in blocking_obstacles:
        obstacle_waypoints = find_safe_waypoints_for_obstacle(obstacle, current_start, end, direction, safety_radius, flight_altitude)
        if obstacle_waypoints:
            if len(obstacle_waypoints) > 1:
                all_waypoints.extend(obstacle_waypoints[1:])
            else:
                all_waypoints.extend(obstacle_waypoints)
            current_start = obstacle_waypoints[-1]
    
    if all_waypoints[-1] != end:
        all_waypoints.append(end)
    
    return merge_adjacent_waypoints(all_waypoints, safety_radius)


def is_path_safe(p1: List[float], p2: List[float], obstacles: List[Dict], flight_altitude: float) -> bool:
    for obs in obstacles:
        if obs.get('height', 30) > flight_altitude:
...
def create_avoidance_path(start: List[float], end: List[float], obstacles_gcj: List[Dict], flight_altitude: float, direction: str, safety_radius: float = 5) -> List[List[float]]:
    ...
    if straight_safe:
        return [start, end]
    return create_multi_waypoint_avoidance_path(start, end, obstacles_gcj, flight_altitude, direction, safety_radius)
...
class HeartbeatSimulator:
    ...
    def export_flight_data(self) -> pd.DataFrame:
        if not self.flight_log:
            return pd.DataFrame()
        data = [{'timestamp': h.timestamp, 'flight_time': h.flight_time, 'lat': h.lat, 'lng': h.lng,
...
def create_planning_map(center_gcj: List[float], points_gcj: Dict, obstacles_gcj: List[Dict], 
                        ...
                        map_type: str = "satellite", straight_blocked: bool = True, flight_altitude: float = 50,
                        drone_pos: Optional[List] = None, direction: str = "智能绕行", safety_radius: float = 5) -> folium.Map:
    ...
    draw = plugins.Draw(export=True, position='topleft',
        draw_options={'polygon': {'allowIntersection': False, 'showArea': True, 'color': '#ff0000',
                      'fillColor': '#ff0000', 'fillOpacity': 0.4}, 'polyline': False, 'rectangle': False,
    ...
    if points_gcj.get('A') and points_gcj.get('B'):
        color = "blue" if not straight_blocked else "gray"
        folium.PolyLine([[points_gcj['A'][1], points_gcj['A'][0]], [points_gcj['B'][1], points_gcj['B'][0]]],
                       color=color, weight=2, opacity=0.4, dash_array='5, 5',
                       popup="直线航线" if not straight_blocked else "⚠️ 直线被阻挡").add_to(m)
    
    if drone_pos:
        folium.Circle(radius=safety_radius, location=[drone_pos[1], drone_pos[0]], color="blue",
                     weight=2, fill=True, fill_color="blue", fill_opacity=0.2,
...
def render_planning_page(drone_speed: int, flight_alt: float, auto_save: bool):
    ...
    with col1:
        render_planning_controls(flight_alt)
    with col2:
        render_planning_map_view(flight_alt, straight_blocked)


def render_planning_controls(flight_alt: float):
    st.subheader("🎮 控制面板")
    ...
    if st.session_state.planned_path:
        ...
        st.metric("📏 规划路径总长", f"{total_dist:.0f} 米")


def render_point_settings():
    st.markdown("#### 🎯 设置方式选择")
    setting_mode = st.radio("选择设置方式", ["✏️ 经纬度输入", "🖱️ 鼠标点击设置"], horizontal=True, key="point_setting_mode")
...
def render_path_strategy(flight_alt: float):
    ...
    strategies = {"智能绕行": "🔄 智能绕行", "向左绕行": "⬅️ 向左绕行", "向右绕行": "➡️ 向右绕行"}
    with col_dir1:
        if st.button(strategies["智能绕行"], use_container_width=True, type="primary" if st.session_state.current_direction == "智能绕行" else "secondary"):
            st.session_state.current_direction = "智能绕行"
            st.session_state.planned_path = create_avoidance_path(
                st.session_state.points_gcj['A'], st.session_state.points_gcj['B'],
                st.session_state.obstacles_gcj, flight_alt, "智能绕行", st.session_state.safety_radius)
            st.success("已切换到智能绕行模式")
            st.rerun()
    with col_dir2:
        if st.button(strategies["向左绕行"], use_container_width=True, type="primary" if st.session_state.current_direction == "向左绕行" else "secondary"):
            ...
            st.rerun()
    with col_dir3:
        if st.button(strategies["向右绕行"], use_container_width=True, type="primary" if st.session_state.current_direction == "向右绕行" else "secondary"):
            ...
            st.rerun()
    st.info(f"📌 当前策略: **{st.session_state.current_direction}** | 安全半径: **{st.session_state.safety_radius}米**")
...
def render_planning_map_view(flight_alt: float, straight_blocked: bool):
    ...
    m = create_planning_map(center, st.session_state.points_gcj, st.session_state.obstacles_gcj, flight_trail,
                           st.session_state.planned_path, "satellite", straight_blocked, flight_alt,
                           drone_pos, st.session_state.current_direction, st.session_state.safety_radius)
...
def handle_map_click(output: Any):
    if output and output.get("last_clicked"):
        ...
        if clicked and isinstance(clicked, dict):
            lng, lat = clicked.get("lng"), clicked.get("lat")
...
def render_obstacle_dialog():
    ...
    with col_ok:
        if st.button("✅ 确认添加", use_container_width=True, type="primary"):
            new_obstacle = {"name": new_name, "polygon": st.session_state.pending_obstacle, "height": new_height,
                           "selected": False, "id": f"obs_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
...
def render_flight_monitoring_page(flight_alt: float, drone_speed: int):
    ...
    if st.session_state.planned_path and len(st.session_state.planned_path) > 1:
        with st.expander("📋 当前规划航线信息", expanded=False):
            waypoint_count = len(st.session_state.planned_path) - 2
            total_dist = calculate_path_length(st.session_state.planned_path) * 111000
            col_info1, col_info2, col_info3 = st.columns(3)
            with col_info1:
                st.metric("绕行策略", st.session_state.current_direction)
            with col_info2:
                st.metric("绕行点数量", waypoint_count)
            with col_info3:
                st.metric("路径总长", f"{total_dist:.0f} 米")
    ...
    if st.session_state.heartbeat_sim.history:
        ...
        with col1:
            ...
            if total_waypoints > 0:
                waypoint_progress = current_waypoint / total_waypoints if current_waypoint <= total_waypoints else 1.0
                st.metric("🎯 当前航点", waypoint_display, delta=f"进度 {int(waypoint_progress*100)}%")
        ...
        with col4:
            remaining = max(0, latest.remaining_distance)
            dist_text = f"{remaining:.0f} m" if remaining < 1000 else f"{remaining/1000:.2f} km"
            st.metric("📏 剩余距离", dist_text)
        with col5:
            eta_seconds = remaining / latest.speed if latest.speed > 0 and remaining > 0 else 0
            if eta_seconds < 60:
                eta_text = f"{eta_seconds:.0f}秒"
            elif eta_seconds < 3600:
                eta_text = f"{int(eta_seconds//60):02d}:{int(eta_seconds%60):02d}"
            else:
                eta_text = f"{int(eta_seconds//3600):02d}:{int((eta_seconds%3600)//60):02d}"
            st.metric("🕐 预计到达", eta_text if not latest.arrived else "已到达")
        with col6:
        ...
        with col_export1:
            if st.button("📊 导出飞行数据", use_container_width=True):
                ...
                if not df.empty:
                    ...
                    st.download_button("📥 下载CSV", csv, f"flight_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        with col_export2:
        ...
        if st.session_state.planned_path and len(st.session_state.planned_path) > 1:
            ...
            total_waypoints = len(st.session_state.planned_path)
            st.success(f"📌 已规划 {total_waypoints} 个航点（包括起点和终点）")
            ...
            with st.expander("📋 查看详细航点列表"):
                ...
                for i, wp in enumerate(st.session_state.planned_path):
                    if i == 0:
                        ...
                        wp_type = f"📍 绕行点 {i}"
                    waypoint_table.append({"序号": i+1, "类型": wp_type, "经度": f"{wp[0]:.6f}", "纬度": f"{wp[1]:.6f}"})
                st.dataframe(pd.DataFrame(waypoint_table), use_container_width=True)


def display_monitor_map(flight_alt: float, latest):
    monitor_map = folium.Map(location=[latest.lat, latest.lng], zoom_start=18, tiles=config.GAODE_SATELLITE_URL, attr="高德卫星地图")
    for obs in st.session_state.obstacles_gcj:
        coords = obs.get('polygon', [])
        height = obs.get('height', 30)
        if coords and len(coords) >= 3:
    ...
    if st.session_state.planned_path and len(st.session_state.planned_path) > 1:
        ...
        folium.PolyLine([[p[1], p[0]] for p in st.session_state.planned_path], color=line_color, weight=3, opacity=0.7,
                       popup=f"规划航线 - {st.session_state.current_direction}").add_to(monitor_map)
    folium.Circle(radius=st.session_state.safety_radius, location=[latest.lat, latest.lng], color="blue", weight=2, fill=True, fill_opacity=0.2,
                 popup=f"🛡️ 安全半径: {st.session_state.safety_radius}米").add_to(monitor_map)
    ...
    if st.session_state.points_gcj['A']:
        folium.Marker([st.session_state.points_gcj['A'][1], st.session_state.points_gcj['A'][0]], popup="起点 A",
                     icon=folium.Icon(color='green', icon='play', prefix='fa')).add_to(monitor_map)
    if st.session_state.points_gcj['B']:
        folium.Marker([st.session_state.points_gcj['B'][1], st.session_state.points_gcj['B'][0]], popup="终点 B",
                     icon=folium.Icon(color='red', icon='flag-checkered', prefix='fa')).add_to(monitor_map)
    ...
    if st.session_state.obstacles_gcj:
        for idx, obs in enumerate(st.session_state.obstacles_gcj):
            with st.container(border=True):
                ...
                with col1:
                    st.write(f"**{obs.get('name', f'障碍物{idx+1}')}**")
                with col2:
                    st.write(f"高度: {obs.get('height', 30)}m")
                with col3:
                    new_h = st.number_input("", value=obs.get('height', 30), min_value=1, max_value=200, step=5, key=f"h_{idx}", label_visibility="collapsed")
...
def main():
    ...
    # 使用当前的高度和速度（可从侧边栏获取，也可从session state获取）
    current_flight_alt = st.session_state.current_flight_altitude
    current_drone_speed = st.session_state.current_drone_speed
    
    # 更新路径当高度变化时
    if current_flight_alt != st.session_state.last_flight_altitude:
        st.session_state.last_flight_altitude = current_flight_alt
        if st.session_state.planned_path is not None:
            st.session_state.planned_path = create_avoidance_path(
                st.session_state.points_gcj['A'], st.session_state.points_gcj['B'],
                st.session_state.obstacles_gcj, current_flight_alt,
                st.session_state.current_direction, st.session_state.safety_radius)
            st.rerun()
    
    if page == "🗺️ 航线规划":
        render_planning_page(current_drone_speed, current_flight_alt, auto_save)
    elif page == "📡 飞行监控":
        render_flight_monitoring_page(current_flight_alt, current_drone_speed)
    elif page == "🚧 障碍物管理":
        render_obstacle_management_page(current_flight_alt)
...
def create_planning_map(center_gcj: List[float], points_gcj: Dict, obstacles_gcj: List[Dict], 
                        flight_history: Optional[List] = None, planned_path: Optional[List] = None,
                        map_type: str = "satellite", straight_blocked: bool = True, flight_altitude: float = 50,
                        drone_pos: Optional[List] = None, direction: str = "最佳航线", safety_radius: float = 5) -> folium.Map:
    tiles = config.GAODE_SATELLITE_URL
    m = folium.Map(location=[center_gcj[1], center_gcj[0]], zoom_start=16, tiles=tiles, attr="高德卫星地图")
...
def render_flight_monitoring_page(flight_alt: float, drone_speed: int):
    ...
    if st.session_state.heartbeat_sim.history:
        ...
        with col3:
            remaining = max(0, latest.remaining_distance)
            st.metric("📏 剩余距离", f"{remaining:.0f} m" if remaining < 1000 else f"{remaining/1000:.2f} km")
        ...
        if st.button("📊 导出飞行数据", use_container_width=True):
            df = st.session_state.heartbeat_sim.export_flight_data()
            if not df.empty:
                st.download_button("📥 下载CSV", df.to_csv(index=False), f"flight_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
    else:
...
def display_monitor_map(flight_alt: float, latest):
    ...
    for obs in st.session_state.obstacles_gcj:
        ...
        if coords and len(coords) >= 3:
            color = "red" if height > flight_alt else "orange"
            folium.Polygon([[c[1], c[0]] for c in coords], color=color, weight=2, fill=True, fill_opacity=0.3).add_to(monitor_map)
    if st.session_state.planned_path:
        folium.PolyLine([[p[1], p[0]] for p in st.session_state.planned_path], color="green", weight=3, opacity=0.7).add_to(monitor_map)
    folium.Circle(radius=st.session_state.safety_radius, location=[latest.lat, latest.lng], color="blue", weight=2, fill=True, fill_opacity=0.2).add_to(monitor_map)
    folium.Marker([latest.lat, latest.lng], icon=folium.Icon(color='red', icon='plane', prefix='fa')).add_to(monitor_map)
    folium_static(monitor_map, width=900, height=500)
