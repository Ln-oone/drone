...
def is_path_safe(p1: List[float], p2: List[float], obstacles: List[Dict], flight_altitude: float, safety_radius: float) -> bool:
    """检查线段是否安全"""
    for obs in obstacles:
        if obs.get('height', 30) > flight_altitude:
            poly = obs.get('polygon', [])
...
def create_curved_waypoints(start: List[float], end: List[float], 
                            boundary_lng: float, offset_deg: float,
                            side: str, num_waypoints: int,
                            start_offset: float = 1.0, end_offset: float = 1.0) -> List[List[float]]:
    """
    创建弯曲的绕行点（非直线排列）
    使用正弦曲线使绕行点呈现弧形
    """
    ...
    # 起点附近（20%范围内）分配1-2个点
    t_values.extend([0.05, 0.1, 0.15][:max(1, num_waypoints // 3)])
    
    # 中间区域分配点
    mid_count = max(1, num_waypoints - 4)
    ...
    # 终点附近（20%范围内）分配1-2个点
    t_values.extend([0.85, 0.9, 0.95][:max(1, num_waypoints // 3)])
    ...
    for t in t_values:
        ...
        # 使用正弦波创造弯曲路径，防止点在一条直线上
        # 添加弧度偏移
        arc_factor = math.sin(math.pi * t)  # 正弦曲线，中间高两边低
        
        # 根据左右绕行调整弧度方向
        if side == "right":
            # 向右绕行时，弧度向更右方向凸出
            arc_multiplier = 1.0 + arc_factor * 0.5
        else:
            # 向左绕行时，弧度向更左方向凸出
            arc_multiplier = 1.0 + arc_factor * 0.5
        
        # 计算绕行点经度
        offset = offset_deg * (start_offset + (end_offset - start_offset) * t) * arc_multiplier
        waypoint_lng = boundary_lng + offset if side == "right" else boundary_lng - offset
        
        waypoint = [waypoint_lng, orig_y]
        waypoints.append(waypoint)
    
    return waypoints


def find_right_avoidance_path_optimized(start: List[float], end: List[float], 
    ...
    # 基础偏移距离 = 安全半径 + 裕量（紧贴障碍物）
    base_offset_meters = safety_radius + 1.0
    offset_meters = base_offset_meters
    
    # 绕行点数量（总航点数 = 绕行点 + 起点终点，控制在5-8）
    total_waypoints_count = 5 + random.randint(0, 3)  # 5-8个总航点
    num_mid_waypoints = max(3, total_waypoints_count - 2)  # 中间绕行点数量
    
    # 尝试不同的偏移距离
    best_path = None
    min_offset = float('inf')
    
    # 起点和终点附近的不同偏移系数（使绕行不对称）
    start_offset_factor = 0.8 + random.random() * 0.4
    end_offset_factor = 0.8 + random.random() * 0.4
    ...
    for attempt in range(1, 12):
        ...
        waypoints = create_curved_waypoints(
            start, end, max_lng, offset_deg_lng, "right",
            num_mid_waypoints, start_offset_factor, end_offset_factor
        )
        ...
        # 检查绕行点是否在一条直线上
        if path_safe and len(candidate_path) >= 3:
            # 检查点是否共线
            points = candidate_path[1:-1]
            colinear = True
            if len(points) >= 2:
                x0, y0 = points[0]
                x1, y1 = points[1]
                for i in range(2, len(points)):
                    x, y = points[i]
                    area = (x1 - x0) * (y - y0) - (x - x0) * (y1 - y0)
                    if abs(area) > 1e-9:
                        colinear = False
                        break
            if colinear and len(points) >= 3:
                path_safe = False
        ...
        # 增加偏移距离
        offset_meters = base_offset_meters + safety_radius * attempt * 0.6
    ...
    # 保底方案：使用弧形大绕行
    fallback_waypoints = []
    extra_offset = (safety_radius + 3) * deg_per_meter_lng
    
    num_fallback = max(3, min(6, num_mid_waypoints))
    for i in range(num_fallback):
        t = (i + 1) / (num_fallback + 1)
        arc_factor = math.sin(math.pi * t)
        offset = extra_offset * (0.8 + arc_factor * 0.4)
        
        orig_x = start[0] + (end[0] - start[0]) * t
        waypoint_lng = max_lng + offset
        fallback_waypoints.append([waypoint_lng, orig_y])
    
    return [start] + fallback_waypoints + [end]


def find_left_avoidance_path_optimized(start: List[float], end: List[float], 
                                        obstacles_gcj: List[Dict], flight_altitude: float, 
                                        safety_radius: float = 5) -> List[List[float]]:
    """
    向左绕行算法 - 优化版
    ...
    # 基础偏移距离（与向右绕行不同，使左右不对称）
    base_offset_meters = safety_radius + 1.5
    offset_meters = base_offset_meters
    
    # 绕行点数量（与向右绕行可能不同）
    total_waypoints_count = 5 + random.randint(0, 3)
    num_mid_waypoints = max(3, total_waypoints_count - 2)
    
    # 起点和终点附近的偏移系数（与向右绕行不同）
    start_offset_factor = 0.7 + random.random() * 0.5
    end_offset_factor = 0.7 + random.random() * 0.5
    ...
    for attempt in range(1, 12):
        ...
        # 创建弧形绕行点（向左）
        waypoints = create_curved_waypoints(
            start, end, min_lng, offset_deg_lng, "left",
            num_mid_waypoints, start_offset_factor, end_offset_factor
        )
...
def find_best_avoidance_path(start: List[float], end: List[float], obstacles_gcj: List[Dict], 
    ...
    left_path = find_left_avoidance_path_optimized(start, end, obstacles_gcj, flight_altitude, safety_radius)
...
def create_avoidance_path(start: List[float], end: List[float], obstacles_gcj: List[Dict], 
    ...
    if direction == "向左绕行":
        return find_left_avoidance_path_optimized(start, end, obstacles_gcj, flight_altitude, safety_radius)
    elif direction == "向右绕行":
...
def create_planning_map(center_gcj: List[float], points_gcj: Dict, obstacles_gcj: List[Dict], 
    ...
    if drone_pos:
        folium.Circle(radius=safety_radius, location=[drone_pos[1], drone_pos[0]], 
                     ...
                     fill_opacity=0.2, popup=f"🛡️ 安全半径: {safety_radius}米").add_to(m)
    elif points_gcj.get('A'):
        folium.Circle(radius=safety_radius, location=[points_gcj['A'][1], points_gcj['A'][0]], 
                     color="blue", weight=2, fill=True, fill_color="blue", 
                     fill_opacity=0.15, popup=f"🛡️ 安全半径: {safety_radius}米 (起点)").add_to(m)
...
def render_sidebar() -> Tuple[str, int, float, bool]:
    ...
    new_safety_radius = st.sidebar.slider("安全半径 (米)", min_value=1, max_value=20, 
    ...
    if new_safety_radius != st.session_state.safety_radius:
        ...
        if st.session_state.planned_path is not None and st.session_state.points_gcj['A'] and st.session_state.points_gcj['B']:
...
def render_communication_page():
    ...
    with col_log_tabs[0]:
        show_gcs_obc_fcu = st.button("📤 GCS → OBC → FCU", use_container_width=True, type="primary")
    with col_log_tabs[1]:
        show_fcu_obc_gcs = st.button("📥 FCU → OBC → GCS", use_container_width=True, type="secondary")
    ...
    with col_btn1:
        if st.button("▶️ 开始飞行", use_container_width=True, type="primary"):
            if st.session_state.points_gcj['A'] and st.session_state.points_gcj['B']:
                ...
                total_dist = calculate_path_length(path) * 111000
                comm.add_planning_record({"message": "开始航线规划", "details": f"算法: A* | 障碍物数量: {len(st.session_state.obstacles_gcj)}"})
                comm.add_planning_record({"message": "航线规划完成", "details": f"类型: horizontal | 航点数: {len(path)} | 路径长度: {total_dist:.1f}m"})
    ...
    if st.session_state.heartbeat_sim.history:
        ...
        with col1:
            ...
            if total_waypoints > 0:
                ...
                st.metric(label="🎯 当前航点", value="0 / 0", delta=None, help="暂无航点信息")
        
        with col2:
            st.metric(label="💨 飞行速度", value=f"{latest.speed:.1f} m/s", 
        ...
        with col4:
            ...
            st.metric(label="📏 剩余距离", value=distance_text if not latest.arrived else "0 m", 
                     delta="已到达!" if latest.arrived else None, help="距离终点的直线距离")
        
        with col5:
            st.metric(label="🕐 预计到达", value=estimated_arrival, delta=None,
        ...
        with col6:
            ...
            st.metric(label="🔋 电量模拟", value=f"{battery_color} {battery_percentage:.0f}%", 
                     delta=f"{latest.voltage:.1f}V", help="模拟电量（基于电压和飞行时间）")
        ...
        with col7:
            st.metric(label="📍 当前位置", value=f"{latest.lat:.6f}, {latest.lng:.6f}", 
                     delta=None, help="当前经纬度坐标")
        
        with col8:
            st.metric(label="📏 飞行高度", value=f"{latest.altitude} m", delta=None, help="当前海拔高度")
        
        with col9:
            st.metric(label="🛰️ 卫星数量", value=f"{latest.satellites} 颗", delta=None, help="GPS卫星信号数量")
        ...
        with col10:
            if latest.arrived:
                ...
                status = "⏸️ 已停止"
            st.metric(label="📌 飞行状态", value=status, delta=None, help="当前任务执行状态")
...
def display_monitor_map(flight_alt: float, latest):
    tiles = config.GAODE_SATELLITE_URL
    monitor_map = folium.Map(location=[latest.lat, latest.lng], zoom_start=18, tiles=tiles, attr="高德卫星地图")
    ...
    if st.session_state.planned_path and len(st.session_state.planned_path) > 1:
        ...
        folium.PolyLine([[p[1], p[0]] for p in st.session_state.planned_path], color=line_color, 
                       weight=3, opacity=0.7, popup=f"规划航线 - {st.session_state.current_direction}").add_to(monitor_map)
    ...
    if st.session_state.planned_path and len(st.session_state.planned_path) > 2:
        for i, point in enumerate(st.session_state.planned_path[1:-1]):
            folium.CircleMarker([point[1], point[0]], radius=4, color="yellow", fill=True, 
                               fill_color="yellow", fill_opacity=0.8, popup=f"航点 {i+1}").add_to(monitor_map)
...
def render_obstacle_management_page(flight_alt: float):
    ...
    with col_data3:
        if st.session_state.obstacles_gcj:
            ...
            st.download_button(label="📥 导出配置", data=json_str, 
...
def render_obstacle_card(idx: int, flight_alt: float, container):
    ...
    with container:
        with st.container(border=True):
            ...
            new_h = st.number_input("调整高度", value=height, min_value=1, max_value=200, step=5, 
